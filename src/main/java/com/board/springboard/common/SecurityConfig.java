...
// 만약에 @Configuration이 아니라 application.yml에 쓴다면
// 키이름은 SecurityConfig : 한 줄로 모든 세팅을 다해야하는데.. 한 줄로는 세팅하는데 한계가 있을 때 사용하는 방법

@Configuration // 설정 파일 세팅 = application.yml 비슷한 효과 application처럼 한줄 설정이 되지 않아
// 코드를 길게 작성하여 프로젝트 환경설정을 해야할 때 작성하는 어노테이션
...
public class SecurityConfig {
    // 파일 같은 경우 -> common에서 수제 제작해서 사용할 것
    private final JwtFilter jwtFilter;

    // 스프링 컨테이너에 보안팀으로 등록하는 역할
    // SecurityFilterChain @Bean 설정을 하면 스프링에 기존 존재하는 보안팀과 우리 회사에서 만든 보ㅓ안팀이 한 팀이되어
    // 나의 프로젝트 지키겠다. 설정
    @Bean
    public SecurityFilterChain 보안설정(HttpSecurity http) throws Exception {
        http
                ...
                .sessionManagement(
                        sm -> sm
                                // 세션을 안만들고,
                                // 기존 방식 : 로그인하면 서버가 세션 생성하여 기억 -> 서버에서도 모든 유저 기억 -> 메모리 소모 및 보안 위험
                ...
                ).permitAll().anyRequest().authenticated() // 이외 나머지 주소는 로그인을 한 후 접근할 수 있다.
                )
                // Controller에서 클라이언트의 접근이 들어오면
